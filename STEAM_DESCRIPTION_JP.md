...
[*] [b]統合設定ウィンドウ[/b]（歯車ボタン）：2 カラム折りたたみ式、変更は即反映・即保存、ESC メニュー不要。UI 一式（設定／検索ウィンドウ、ミニマップ外枠とボタン列）はファミリー共通の角丸ダークスキン
...
`registerZoneProvider` ゾーン描画フレームワークも提供；[url=https://steamcommunity.com/sharedfiles/filedetails/?id=3768276209]Zones addon[/url] でサーバー定義ゾーン（zones.json）を表示可。
