...
0.20.0 以降は [url=https://steamcommunity.com/sharedfiles/filedetails/?id=3789836701][b]Minidoracat UI Library for B42[/b][/url] が必須です。Required Items から併せて購読してください。未導入では本 MOD を読み込めません。
...
[*] [b]マップ表示設定[/b]（歯車）：左に分類、右に設定＋横断検索。分割画面／大きい文字は1ペイン。即反映・保存、ESC不要；設定／検索／ミニマップは共通の角丸ダークスキン、項目別の性能説明と対策付き
...
バニラの POI 1669件・20分類（軍警／医療／商業／工農業など）を色分け。染色シルエット／フルカラーアイコンか半透明区画を選び、分類別に切替可。位置は公式地図、分類は部屋の[u]loot用途[/u]由来。地図の用途別色分けと異なる場合があり、実際の物資が基準です。
