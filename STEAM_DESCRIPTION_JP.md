...
[h2]🧰 主な機能[/h2]
[list]
[*] [b]ナビ目標＋道路ルート[/b]：右クリックで設定。道路沿いに案内し、逸脱時に再計算、荒野は最寄り道路へ。旗＋端の矢印＋距離、到着で解除；派閥共有で仲間にもルート表示
[*] [b]マップ検索[/b]：虫眼鏡か右クリックで座標・通り名（[u]原名／翻訳名[/u]）・施設20種を距離順検索。地図へジャンプ（金色パルスマーカー）かナビ設定。「MOD マップ：名前」は確認済みの出所表示で、不具合の印ではありません
...
[*] [url=https://steamcommunity.com/sharedfiles/filedetails/?id=3792675881]AutoDrive[/url]——GPS ナビ、運転 HUD、道路網に沿った自動運転
[/list]
