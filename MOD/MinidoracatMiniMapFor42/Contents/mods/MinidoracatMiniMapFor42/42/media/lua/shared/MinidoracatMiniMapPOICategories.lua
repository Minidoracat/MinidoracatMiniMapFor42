...
--   （<1.5px/格只留圖標、<6px/格聯集框純填色（無框線無名稱）、>=6px/格逐
--   房間平面圖＋框線＋名稱，訊號 mapAPI:getWorldScale()，原版標記同機制）；
--   圖標於 <6px/格 做同格去重疊（同一圖標尺寸螢幕格只畫第一顆——拉遠時
--   數百顆互疊圖標是遠距檔最大殘餘成本，疊格視覺上不可分辨）；三 pass 投影
--   改仿射快取（每 pass 3 點採樣、逐矩形純 Lua 乘加，中距檔實測 244 FPS）。
--   無 lodRect 的外部 zone 不參與任何 LOD/去重疊（Zones addon 對建物尺度
--   區域自動附 lodRect，大範圍區域不附——見其 attachLodRect）。
--
