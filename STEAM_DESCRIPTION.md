...
[*] [b]玩家座標顯示＋一鍵複製[/b]：小地圖底部置中顯示目前 x, y, z（可關）；按鈕列 XY 鈕一鍵複製座標到剪貼簿、右鍵選單可複製指向處座標——貼上即用於 /teleportto 等指令
[*] [b]穿透模式（點擊穿透）[/b]：點擊／滾輪／右鍵全部穿透小地圖直達遊戲世界，地圖本體半透明（不透明度滑條可調）——拉大當常駐畫面不擋操作、不擋視線；快捷鍵 '、浮動圖標右鍵或設定皆可切換
