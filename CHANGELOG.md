...
- **管理員檢視旁路（雙層政策，預設全關）**：沙盒新增「允許管理員戰術檢視旁路」與
  ...
  設定視窗勾選；少任何一把都不旁路。權限採 B42 的 Role＋Capability 判定，不看已
  deprecated 的 `isAdmin()`／`getAccessLevel()`，自訂 Role 一樣適用。
  ...
  - 旁路生效期間，小地圖與世界地圖左上角常駐「管理員檢視」（英文介面 `ADMIN VIEW`）
    標記，避免管理員忘記自己開著看穿模式。
  - 勾選狀態只存在 Policy facade 的本機私有 slot table，逐分割畫面 slot 獨立，不寫
...
- 全服沙盒政策讀取收斂到單一 shared facade（`MinidoracatMiniMap_Policy.lua`）：真相源
