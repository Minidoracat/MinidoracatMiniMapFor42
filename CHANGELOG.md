...
- **資源點區塊視覺改版：框線退場、暗色描邊上場**：區塊不再畫邊線，改為
  ...
  難辨識，halo 是清晰度與效能的折衷）
- **全面效能優化**（實測 244 FPS 場景整輪驗證）：區域渲染三 pass 各加
  空跑閘（無填色／框線／圖標的資料集整段跳過）；熱路徑比較鏈取代
  math.min/max（省 Kahlua→Java 跨界呼叫）；殭屍／動物點位投影改仿射
  快取——每幀只做 6 次 Java 座標取樣、其餘全走純 Lua 乘加；小地圖隱藏時
  渲染鏈早退（導航抵達判定仍照常執行）；玩家座標列快取（同格不重算）；
  POI 重建簽章 15-tick 節流＋選項快取
...
> 技術要點：同棟合併是兩段式分組——先精確 bbox 相等、再以 union-find 合併
