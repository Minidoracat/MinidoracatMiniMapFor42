...
> 技術要點：`drawSafehouses` 改為 1 秒幾何快取（清單筆數、帳號、陣營變動立即重建），距離閘先於 `playerAllowed`，成員結果延遲查詢並隨快取重建；夾值改比較式以避開 Kahlua `math.*` 跨界呼叫。任一已開圖層沒有距離閘時，以主檔 `visibleWorldAABB` 做視野裁切，與外接框不相交的列整列跳過；全部圖層都有距離閘時不計算外接框。
