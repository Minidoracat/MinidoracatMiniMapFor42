...
> 技術要點：新增 `MinidoracatMiniMap_Places.lua`，資料存角色 modData `MinidoracatMiniMapPlaces`（schema 1），MP 沿用 `transmitModData`。回家經 `Core.navSetTarget` 取代成單站；會丟掉 ≥2 個待前往站或行程資料損壞時走既有取代確認，多站 claim 中回 `busy`、行駛中回 `notstopped`，不開必定失敗的確認窗。導航 API 升為 v8（additive）：`getNavHome`、`goNavHome`；既有介面與行程語意不變。
