...
> 技術要點：`drawSafehouses` 改為 1 秒幾何快取（清單筆數、帳號、陣營變動立即重建），距離閘先於 `playerAllowed`，成員結果延遲查詢並隨快取重建；夾值改比較式以避開 Kahlua `math.*` 跨界呼叫。任一已開圖層沒有距離閘時，以主檔 `visibleWorldAABB` 做視野裁切，與外接框不相交的列整列跳過；全部圖層都有距離閘時不計算外接框。內部 POI provider（原子換表）的候選快取不再走 1 秒 TTL，改由表身分、視窗溢出、縮放、移動與距離閘參數失效；外部 provider 維持 TTL。

## [42.20.4-0.28.6] - 2026-09-25

### 修正

- 只設一個導航目標、正在自動駕駛時，無法在地圖上換目標（會跳出「自駕正在控制車輛，請先停止自駕」）。現在單一目標可以邊開邊換，自駕會直接改開往新目標；多站行程仍需先停止自駕才能修改。

## [42.20.4-0.28.5] - 2026-09-25

### 修正

- 地圖顯示設定視窗裡，其他 MOD（例如自動駕駛）加進來的下拉選單，滑鼠移過去不會顯示說明。現在會正常顯示。
- 行程導航中，按下停止自動駕駛或手動接手開車時，導航路線（畫面上與小地圖）會一起消失。現在只交還駕駛，導航照常顯示，可以照著路線自己開，或停車後再啟動自動駕駛。

> 技術要點：`releaseNavLeg` 的 reason=`manual` 不再把行程改為 paused，保留 navigating 並換發未接管的新段 token；其他 reason 行為不變。

## [42.20.4-0.28.4] - 2026-09-24

### 變更

- 地圖選項「等軸測」的中文名稱統一為「等軸測投影」，與本體中文翻譯包一致；同時安裝多個中文翻譯時，不會再因載入順序出現不同名稱。

## [42.20.4-0.28.3] - 2026-09-24

### 修正

- 修正部分道路的導航路線偏離路面：例如 Echo Creek 的 Elder Road，路線整段斜穿南側草地；雙向分隔道路每隔一段就斜穿中央分隔帶。自動駕駛會照著這條線開下路面、撞上草地或分隔帶的樹。現在路線沿實際路面。更新後請重啟遊戲。

> 技術要點：導航建圖在 300 格街道覆蓋邊界預切路段，切點原本被當成街道端點去吸附附近路口，連帶把整段街道拖離路面；現在兩側都保留的切點視為街道連續、不再移動，被地圖 MOD 覆蓋裁掉的接縫仍可吸附接線。全圖偏離路面超過 3 格的路網邊由 85 段降到 46 段（未切段時為 45 段），連通結構不變。導航 API 與存檔不變。

## [42.20.4-0.28.2] - 2026-09-16

### 修正

- 修正搭配 Extraction Mode、顯示突襲邊界時，世界地圖與小地圖右鍵選單沒有反應的相容性問題；保留邊界與撤離點標記，不影響對方自訂的右鍵功能。更新後請完整重啟客戶端。

## [42.20.4-0.28.1] - 2026-09-14

### 修正

- 配合共用介面函式庫的置頂修正，小地圖浮動入口維持一般視窗層級，避免遮住後開的設定視窗；拖曳、位置記憶與重生後恢復不變。更新後請完整重開客戶端。

## [42.20.4-0.28.0] - 2026-09-13

### 新增

- 多目標行程：最多 16 點，可加到最後、指定插入位置、先去優先目標、重排、移除／復原、略過與預覽。新行程預設自動接續，也可逐點等待；標成停靠的點一定等你，舊行程保留手動。
- 搜尋視窗新增行程頁、收合與可重綁的分號快捷鍵；長名稱按寬度換行，小視窗可展開更多操作，取代／清空行程先確認。
- 行程頁與新版 AutoDrive HUD 共用接續模式。普通點停妥後備好下一段再接續；停靠點按「繼續自駕」才走，僅到道路終點不假報抵達。

### 修正

- 小地圖浮動開關由 32×32 放大至 40×40，與公告、交易圖標尺寸一致。
- 修正死亡後直接以新角色繼續遊戲時，小地圖浮動開關消失；恢復原位置並保留顯示設定，更新後請重啟客戶端。
- 浮動圖標位置改為拖曳放開立即保存，依解析度分別記憶；首次升級會沿用舊位置，已有新版記錄不覆蓋。舊座標文字欄位退役，顯示開關不變。
- 修正行程清單的站名與狀態文字重疊；狹小視窗或大字級下，狀態、座標與距離會分行，不再互相遮住。
- 已抵達與已略過分開顯示；略過編號改為深底白字。長狀態、預覽與錯誤全文可逐行捲動閱讀，插入長站名前先提供全文確認。
- 搜尋未選到地點時不再亮出無效動作；認領中的目前點及跨過它的重排會停用，合法後續編輯與停靠偏好仍可調整。
- 修正插入位置與更多動作選單跑到視窗標題下方；改為貼著觸發按鈕，下方不足時往上展開，分割畫面不越界。
- 「自己走過去」改名為「改用直線指引」，補上用途說明；同一顆按鈕可直接切回道路導航，保留目標與行程並從目前位置重算，不會自動走路或自駕。更多選單保存開啟時的切換方向，過期選項不會反向操作。
- 多站行程預設在小地圖與世界地圖顯示後續路段，不必先按預覽才能看見停靠點之間的連線；單人暫停看大地圖時也會繼續計算，仍可手動關閉。

- 地圖搜尋現在會區分「沒有符合結果」、「目前沒有街道資料」與「街道資料載入失敗」，並提示改用座標／設施類別或重新進入存檔再試；街道資料故障不會阻擋座標或可用的設施搜尋。更新後請重啟遊戲。

> 技術要點：導航 API v7、行程 schema 2；autoContinue 與每點 pause 同存角色資料，舊 schema／單點遷移為逐點手動，重進存檔不自動發車。gate 回呼後重驗版本，取消不會被舊候選覆寫。多站自駕請一起更新兩包；v6 僅保留手動行程整合。本次不改道路或車輛控制律；真模組回歸、原生字型離線繪製及 Kahlua Debug 編譯不等同實機視覺／駕駛驗收。

## [42.20.4-0.27.2] - 2026-09-11

### 修正

- 修正 S 1st St／Dixie Highway 穿越 1394 號公路時，導航線繞向側邊再折返的問題；補上路口的南北直通連接，小地圖導航與自動駕駛共用正確路線，原有東西向道路維持連通。
- 修正 River Walk Road 河岸彎道的導航線切過草地；依實際礫石路面補回轉折，保留原有道路連接。回報：[AutoDrive #5](https://github.com/Minidoracat/MinidoracatAutoDriveFor42/issues/5)。

### 其他

- 新增道路／導航線問題的 GitHub 回報表單；附上含座標的導航截圖與文字座標即可，不強制提供自動駕駛診斷紀錄。簡介已補上入口，車輛控制異常仍使用 AutoDrive 原有回報管道。

> 技術要點：導航 API 維持 v5，最低遊戲版本與存檔不變；本版只修正主 MOD 的道路資料，不改 AutoDrive 駕駛控制。更新 MiniMap 後請重新啟動遊戲。已完成離線路網與行駛剖面驗證，尚未完成遊戲內實車驗收。

## [42.20.4-0.27.1] - 2026-09-11

### 修正

- 修正統一漢化、自家中文翻譯與同型語言包已載入街道，導航補路卻沒有生效、短路被導成長距離繞行的問題；改為辨識完整的原版道路副本，不再以語言包的地圖名稱判斷，並保留真正地圖 MOD 的覆蓋規則。
- 支援自家翻譯包只翻街名的新方式；中文與英文搜尋共用現行道路位置，不再因舊位置補充而多出失效結果。

> 技術要點：導航 API 維持 v5，不改 AutoDrive 控制或存檔。更新後需重啟遊戲；採純文字街名的 LangFor42 42.20.4-1.25.0 與本體 42.20.4-0.27.1 請一起更新，以保留中文搜尋。已完成離線路線與行駛剖面驗證，未宣稱遊戲內實車驗收。

## [42.20.4-0.27.0] - 2026-09-10

### 變更

- MOD 地圖的街名翻譯改為只處理名稱，避免翻譯資料落後時連道路座標一起停在舊版；原名與譯名搜尋使用相同的即時道路位置。小地圖本體 42.20.4-0.27.0 與地圖包 42.20.4-0.9.0 或更新版本需一起更新並重啟遊戲。
- 道路修正與街名翻譯分開，恢復雛菊郡 44 條已知輪廓道路的中心線；各語系的地圖顯示與導航使用同一修正，上游改動後不盲套舊座標。
- 馬爾德勞 1993 的完整道路複本改為只抑制確定重複的顯示，不再為了藏路名刪除導航道路；參照尚未載入、被遮蔽或資料不符時保留原狀，部分重疊仍可能顯示兩個名稱。
- 街道搜尋結果新增「MOD 地圖：地圖名稱」來源提示，提供繁中、簡中、英文與日文；只標示可確認的來源，不改地圖街名顏色或導航路線。

> 技術要點：導航介面維持 v5，不改自動駕駛控制邏輯、沙盒選項或存檔。本次只發布小地圖本體與地圖包兩包，自動駕駛／翻譯包／介面函式庫／區域與相容 addon 都不需要同步更新。

## [42.20.4-0.26.6] - 2026-09-09

### 修正

- 修正部分 MOD 地圖的一筆單點街道資料，會讓整個世界的導航與自動駕駛都無法取得路線；現在略過無法構成線段的資料，其餘正常道路仍可使用。
- 修正 Tikitown 等 MOD 地圖的對外幹道，在地圖覆蓋邊界被過度裁切而無法導航的問題；保留原版與 MOD 道路的接線，真正重疊區域仍依地圖優先順序處理。

> 技術要點：導航 API 維持 v5，不改 AutoDrive 控制邏輯或存檔。已完成離線路網與實際街道資料驗證；MOD 地圖的遊戲內駕駛仍待玩家驗收，更新後須重啟遊戲。

## [42.20.4-0.26.5] - 2026-09-09
...
> 技術要點：空街道容器保持 `idle`，由每個地圖實例各自節流重試；搜尋每幀查現行小地圖與世界地圖，不保存舊 UI 參考。補載只在確認容器為空時執行；探測／載入異常保留原因，每實例記錄一次。公開導航 API 維持 v5、參數與路線格式不變，但 `idle` 亦涵蓋空容器等待重試，契約見 `docs/addon-api.md`。原始 MP 回報中街道資料未載入的上游原因仍未確認，本版修正的是讀到空資料後無法恢復的行為。
