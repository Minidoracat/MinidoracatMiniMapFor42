...
- **內建資源點（POI，0.8.0+）**：原版地圖 1669 筆資源點、20 類（軍事／警察／槍店／醫療／
  ...
  ESC 選項頁獨立「資源點（POI）」群組
- **世界地圖圖標**（預設關）：殭屍點位／野生動物／畜養動物／載具四個獨立開關，
  在世界地圖（M）顯示同款圖標；風格、顏色與篩選跟隨小地圖設定；世界地圖按鈕列
  新增爪印按鈕直開設定視窗
- **統一設定視窗**（齒輪鈕）：雙欄八區塊（圖層顯示／資源點／顯示距離／殭屍點位／動物圖標／
  載具圖標／世界地圖圖標／外觀與行為）可收合、全部展開／收合、收合時標題列顯示現況摘要；
  裝 Zones addon 時另有「伺服器區域」區——依 zones.json 實際類別動態生成勾選篩選
...
| 穿透模式 | 關 | 點擊/滾輪/右鍵點擊穿透到遊戲世界＋地圖半透明＋外框變淡＋琥珀邊框；快捷鍵 `'` 或浮動圖標右鍵亦可切換；穿透中小地圖按鈕列收合，退出走熱鍵/浮動圖標右鍵/ESC 選項頁（三層防鎖死） |
...
| 沙盒選項 | 預設 | 說明 |
|----------|------|------|
| 【全域】全部資訊顯示距離上限（格） | 0＝不限制 | 最優先生效：殭屍/動物/載具/安全屋/資源點五類距離的全域上限；個別距離選項只能設得更嚴（兩者取較小），不能放寬 |
...
玩家端另有統一設定視窗「顯示距離」區：五類距離各一滑條（0＝不限）。**伺服器有設限時
