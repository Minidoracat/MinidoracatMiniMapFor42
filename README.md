...
- **按鈕列**：M（世界地圖）、-／+（縮放）、◇（視角切換：等軸測／俯視，與統一
  視窗「等軸測」勾選即時同步）、C（回到玩家）、XY（複製座標）、齒輪（統一設定
  視窗）、X（關閉）；按鈕動態間距，最小尺寸也裝得下
...
**導航目標**：右鍵小地圖或世界地圖（M）→ 「設定導航目標」。目標在視窗內顯示金旗、
超出視窗時沿邊緣顯示方向箭頭與直線距離（小地圖與世界地圖同步顯示）；走到目標
5 格內自動抵達清除；目標隨角色存檔保留。加入陣營（faction）後右鍵選單多
「分享目標給陣營」——同陣營成員的小地圖與世界地圖會以青旗＋名字顯示你的目標
（由伺服器過濾轉送，非同陣營玩家收不到）。
