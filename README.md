...
- **浮動開關圖標**：常駐小圖標點擊開關小地圖（免快捷鍵）；可拖曳擺放、
  拖曳放開即存入原版 `layout.ini`，每種解析度各自記憶，重生與重新進入皆可還原；
  設定可關，開關為 40×40，與公告／交易圖標一致。首次升級會帶入舊位置，已有新版版面記錄優先；
  舊的手動座標欄位已移除，不再有兩份互相覆蓋的位置來源
...
- **世界地圖圖標**（預設關）：殭屍點位／野生動物／畜養動物／載具四個獨立開關，
  ...
  新增爪印按鈕直開設定視窗
- **多目標行程**：最多 16 點，搜尋／雙地圖右鍵可加到最後、指定插入位置或「先去這裡」，
  行程頁可重排、移除／復原及略過。新行程預設自動接續，也可切逐點停等；
  個別目標標成停靠後，即使自動模式也會等你。舊行程保留逐點手動。
  多點預設顯示剩餘路線，可關閉預覽；雙地圖共用目前段青線、後續淡線與編號標記。
...
- **地圖搜尋**：小地圖與世界地圖按鈕列放大鏡鈕、雙地圖右鍵「搜尋地圖…」——
  ...
  放大鏡視窗新增「行程」頁；`;` 可開關視窗（可重綁），支援收合、鍵盤焦點與手把操作。
- **家與收藏地點**：雙地圖右鍵「設為家」「加入收藏…」（可命名，不限筆數）；「回家」一鍵把行程換成
  單站回家並開始導航，入口有小地圖房子鈕、雙地圖右鍵、行程頁與 AutoDrive 儀表 HUD。會丟掉 2 個以上
  待前往點時先確認；單站自駕中直接改開回家。搜尋視窗未輸入時列出收藏，可改名、移除或設為家；
  家與收藏畫在地圖上，隨角色保存（MP／分割畫面各自一份，角色死亡不繼承）。
...
- **家族圓角深色 UI**：搜尋視窗、統一設定視窗與小地圖整框（卡片式外框＋標題列
  ...
  自動退回原版直角樣式，功能不受影響
- **Extraction Mode 相容（0.28.2 起）**：突襲地圖的被動標記覆蓋層不再擋住大小地圖右鍵；
  保留 raid 邊界與撤離點，不修改訂閱檔案，也不覆蓋上游自訂右鍵處理。
...
接目標；無路可在行程頁選擇手動直線指引。超出視窗時只有目前站顯示方向箭頭與距離，
其他站以編號顯示。徒步進入目標 5 格內才完成；車上另須停妥，自駕則等待停妥回報。
行程隨角色保存，重新進入存檔須明確恢復，不會自行發車。
路線內部同時保留逐段道路寬度與 `paved`／`gravel`／`dirt`／`unknown` 路面分類；
路面分類供導航成本使用，寬度則作為相容 addon 的逐段 metadata。route 另帶
`snapDist`（目前位置到路線最近點的投影距離），供導航 addon 判斷接線可用性。
...
地圖搜尋會分開提示查無命中、目前沒有街道資料與街道資料載入失敗。
資料不可用時可改搜座標或設施類別；載入失敗另提示重新進入存檔再試，
不會把故障誤報為「無結果」，也不會清除原有導航目標。

**行程操作**：「加到行程最後」保留原順序；「插入位置」選擇要插在哪個待前往目標之前；
「先去這裡」把新點放在待辦最前並開始導航，但不直接啟動自駕。
原「設定導航目標」仍會取代成單點並立即指引，有行程時先確認；不另設一個終點欄位。
行程頁與 AutoDrive HUD 共用「自動／逐點」模式；想補給或下車，先將該點標成停靠。
切換模式不發車、不煞車，也不改目前目標；重新進入存檔仍須明確恢復。
修改目前點或把其他點移到它之前，須先停止自駕並停車；合法後續編輯不影響本段。
狀態分開標示目前目標、後續數量、已抵達與已略過；長狀態與錯誤全文可在清單逐行閱讀。
插入選單的長名稱會先顯示短標籤，選取後可閱讀完整名稱並確認。
次要動作集中在「動作…」選單，搜尋結果與行程站點也可按右鍵開出同一份動作；選單貼著觸發處展開，下方空間不足才往上，分割畫面限制在發起玩家的區域。
「改用直線指引」只切換指引方式，不會自動移動角色或開車；適用於沒有可用道路路線或想自行越野時。
直線模式時同一顆按鈕會改為「改用道路導航」，保留目標與行程，從目前位置重新規劃；不必先停止導航，也不會啟動自駕。車上須先停妥並解除自駕接管。
預覽僅在需要時分段計算，估計距離含兩端接線；未完成或無路不冒充完整道路。
AutoDrive 在普通自動點停妥、重新備好下一段後接續；明確停靠或逐點模式需按「繼續自駕」。
多站自駕需 MiniMap `42.20.4-0.28.0+` 與 AutoDrive `42.20.4-0.8.0+`，更新後請完整重啟遊戲。
只到道路終點而未抵達原座標時，本站保持待辦，可手動前往或明確略過。
停止導航／自駕只代表交還控制，**不是緊急煞車**；不支援過站不停或自動最佳順序。
...
分享只授權目前這一站；完成、暫停或更換後撤回，下一站須重新分享。
