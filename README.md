...
- **地圖搜尋**：小地圖與世界地圖按鈕列放大鏡鈕、雙地圖右鍵「搜尋地圖…」——
  輸入自動判別座標（`12895,3499` 等格式）／街道名（**中英雙語可搜**；純文字翻譯
  使用即時原名，舊式漢化另有英文補充）／設施類別（20 類），結果依距離排序；可跳大地圖（金色
  脈動標記 12 秒）或直接設為導航目標。可辨識的 MOD 街道會另列「MOD 地圖：地圖名稱」
  （四語），滑鼠停留可查看完整名稱；
  此提示只說明資料來源，不代表該道路有錯，也不改導航位置。
...
裝了第三方「統一中文漢化（B42Trans_CN）」時，若多人客戶端的正規流程沒有載到任何街道，
本 MOD 會補載（繁中／簡中介面載它的中文街名，其他介面語言載官方英文），不需要改裝
本作者的翻譯包；console 會留一行 `carrier fallback` 供支援判讀。

MOD 地圖街名翻譯改為**只翻名稱、保留原作者道路**：所有語系都讀取地圖原本的
`streets.xml`，不再使用含另一份座標的語系 XML。原名與譯名都能搜尋，且使用相同的
即時道路座標；新街道缺譯時保留原名。主 MOD 42.20.4-0.27.0 與 MOD 地圖包 42.20.4-0.9.0
或更新版本需一起更新並重啟遊戲。
已確認的道路問題改由**語系無關的獨立修正資料**處理：恢復雛菊郡 44 條輪廓道路的中心線，
地圖顯示與導航使用同一修正。馬爾德勞 1993 的完整複本僅在同張地圖已有未被遮蔽的參照時
抑制重複顯示，不刪導航道路；參照不明、部分重疊或上游幾何已改動時保留原資料。
