...
- **按鈕列與設定**：按鈕列為 M（世界地圖）、-／+（縮放）、C（回到玩家）、
  =（圖層開關選單）、齒輪（**詳細設定視窗**——尺寸/按鈕列/殭屍點樣式/透明度等
  免進 ESC 選項頁直接調，與 ESC 頁雙向同步）、X（關閉）；按鈕動態間距，
  最小尺寸也裝得下
- **圖層選單**（= 鈕）：原版圖層開關外加「即時殭屍點位」「顯示街名」「鎖定小地圖」
  「安全屋範圍」（與 MOD 選項雙向同步、勾選即存檔）；選單改掛頂層，
  小地圖縮很小時不再被裁切遮住
...
| 拖曳自由查看 | 開 | 拖動小地圖後**停留在該處**（原版放開即回中），點一下小地圖回到玩家 |
...
| 顯示地名 | 開 | 城鎮與區域名稱（開啟時自動連帶開啟齒輪面板「符號」）；**角落小地圖預設看不到文字**——需同時開啟下方「文字註記」實驗選項，世界地圖（M）則不受限 |
